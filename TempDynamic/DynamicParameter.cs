...
namespace TempDynamic
{
    public class DynamicParameter
    {
        ...
        public DynamicParameter(string name, Type parameterType, IEnumerable<CmdletMetadataAttribute> attributes)
        ...
        public static implicit operator RuntimeDefinedParameter(DynamicParameter rp)
        {
        ...
        public T GetChosenValue<T>()
        {
            ...
            return (T)BackingParameter.Value;
        }
        /// <summary>
        /// Retrieves the selected parameter value(s) as a single-dimensional array.
        /// </summary>
        /// <typeparam name="T">
        ///     The type to return the selected value as.  This should match the type defined in <see cref="ParameterType"/>.
        /// </typeparam>
        /// <returns>
        ///     A single-dimensional array of values selected by the parameter casted to the parameter type 
        ///     <typeparamref name="T"/>.  If no value was selected, an empty array is returned.
        /// </returns>
        /// <exception cref="ArgumentException">
        ///     <typeparamref name="T"/> does not match the element type of array type <see cref="ParameterType"/>.
        /// </exception>
        public T[] GetChosenValues<T>()
        {
            if (null == this.BackingParameter?.Value)
                return new T[0];
            ...
            if (this.BackingParameter.Value is ICollection icol && icol.Count > 1)
            {
                return (T[])icol;
            }
            else
            {
                return new T[] { (T)this.BackingParameter.Value };
            }
        }
